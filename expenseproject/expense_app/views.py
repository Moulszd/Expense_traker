...
def index(request):
    ...
    return render(request, 'expense_app/index.html', {
        ...
        'total_expense': total_expense, 'yearly_sum':yearly_sum,
        'weekly_sum':weekly_sum,'monthly_sum':monthly_sum,
        'daily_sums':daily_sums, 'categorical_sums':categorical_sums})

def edit(request, id):
    expense = Expense.objects.get(id=id)
    expense_form = ExpenseForm(expense)
    if request.method =="POST":
    ...
    return render(request, 'expense_app/edit.html', {
        'expense_form':expense_form})

def delete(request, id ):
    if request.method == 'POST' and 'delete' in request.POST: # we add 'and' to avoide bugs from confusing btw post edit and delete
        # expense = Expense.objects.filter(pk=id).delete()
        expense = Expense.objects.get(id=id)
        expense.delete()
    return redirect('index')
